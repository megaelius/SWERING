...
model.10x10CV <- train (as.factor(explicit) ~., data = x, subset=learn, method='nnet', 
...
for (i in 1:k){
    ...
    set.seed(1234)
    mod3 <- nnet(ytraining ~., data = xtraining, entropy = TRUE, softmax = FALSE, linout = FALSE, size=10, maxit=500, decay=0.5)
    options(digits=4)
...
for (i in 1:nrow(xtest)){
  if(pred_nn[i] != explicit[i] && pred_glm[i] != explicit[i] && pred_dico[i] != explicit[i]){
    index = c(index,i)
  }
}
```
